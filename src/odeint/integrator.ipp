...
Integrator<EquationType>::Integrator(const StepBasePtr& stepper,
		                             const ObsBasePtr&  observer,
		                             const Traits& traits) :
	traits_(traits), stp_ptr_(stepper), obs_ptr_(observer) {
	ASSERT(nullptr != stp_ptr_);
...
void Integrator<EquationType>::init_dt(const Time& t, State& u, Time& dt) const{
	...
	auto eqn_ptr = this->stp_ptr_->getEquationPtr();
